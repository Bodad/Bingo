...
//        logManager.log("Playing number {} {} {}", number, hasNumber ? "check":"", hasBingo ? " BINGO":"");
    ...
    public boolean checkForWin() {
        boolean allowStandardBingo = false;
        boolean allowPostageStamps = false;
        boolean allowDiamonds = true;
        boolean allowCorners = false;

        boolean won = false;

        if (allowStandardBingo && checkStandardBingo()) return true;
        if (allowPostageStamps && checkPostageStamps()) return true;
        if (allowDiamonds && checkDiamonds()) return true;
        if (allowCorners && checkCorners()) return true;
